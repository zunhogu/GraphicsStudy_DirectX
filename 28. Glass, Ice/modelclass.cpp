...
bool ModelClass::Initialize(ID3D11Device* device, char* modelFilename, WCHAR* colorTextureFilename, WCHAR* normalTextureFilename) {
	...
	// 이 모델의 텍스처를 가져온다. 
	result = LoadTexture(device, colorTextureFilename, normalTextureFilename);
	if(!result) { 
		return false; 
	}
...
bool ModelClass::LoadTexture(ID3D11Device* device, WCHAR* colorTextureFilename, WCHAR* normalTextureFilename)
{
	bool result;
