...
bool TextureShaderClass::Render(ID3D11DeviceContext* deviceContext, int vertexCount, int instanceCount, XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix, ID3D11ShaderResourceView* texture) {
...
bool TextureShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename) {
	...
	polygonLayout[2].SemanticName = "TEXCOORD";   	// SemanticName 은 범용 시맨틱이기 때문에 TEXCOORD가 된다.
	polygonLayout[2].SemanticIndex = 1;   	// 이미 위에서 텍스처 좌표에 사용하는 TEXCOORD가 0을 사용하고 있어서 SemanticIndex는 1로 설정한다.
	polygonLayout[2].Format = DXGI_FORMAT_R32G32B32_FLOAT;   // 인스턴싱을 위해 레이아웃에 세 번째 요소를 추가한다. 4개의 인스턴싱된 삼각형을 다른 3D 위치에 그릴 예정이기 때문에 셰이더에 추가적으로 x, y, z로 이루어진 벡터 정보를 보내야한다. 따라서 그 포맷은 DXGI_FORMAT_R32G32B32_FLOAT이 된다.
	polygonLayout[2].InputSlot = 1;
	polygonLayout[2].AlignedByteOffset = 0;   // 인스턴스 버퍼의 첫번재 유닛이고 정점버퍼와 인스턴스 데이터를 서로 정렬할 필요가 없기때문에 0으로 설정
	polygonLayout[2].InputSlotClass = D3D11_INPUT_PER_INSTANCE_DATA;   // InputSlotClass를 D3D11_INPUT_PER_INSTANCE_DATA 라고 표시하여 인스턴싱 데이터라는것을 알려준다.
	polygonLayout[2].InstanceDataStepRate = 1;   // 인스턴스 데이터 하나당 하나의 인스턴스를 그리게 한다. 
...
void TextureShaderClass::RenderShader(ID3D11DeviceContext* deviceContext, int vertexCount, int instanceCount)
{
	...
	// 삼각형을 그린다. 이때 기존의 DrawIndex 함수가 아닌 DrawInstanced 함수를 사용한다.
