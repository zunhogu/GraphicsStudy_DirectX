...
bool GraphicsClass::Render() {
	...
	// 텍스처의 이동값을 0에서 1로 증가시킨다.
	// 이동값이 1이상이 되면 다시 0으로 만든다.
	...
	result = m_TextureShader->Render(m_D3D->GetDeviceContext(), m_Model->GetIndexCount(), worldMatrix, viewMatrix, projectionMatrix, m_Model->GetTexture(), textureTranslation);
	if (!result) { 
		return false;
	}
