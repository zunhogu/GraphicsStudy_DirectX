...
bool GraphicsClass::Initialize(int screenWidth, int screenHeight, HWND hwnd) {
	...
	result = m_Model->Initialize(m_D3D->GetDevice(),"../Engine/data/Cube.txt", L"../Engine/data/Fieldstone_DM.tga", L"../Engine/data/fieldstone_SM.tga", L"../Engine/data/fieldstone_NM.tga");
	//result = m_Model->Initialize(m_D3D->GetDevice(), "../Engine/data/Cube.txt", L"../Engine/data/stone02.dds", L"../Engine/data/spec02.dds", L"../Engine/data/bump02.dds");
	if (!result) {
	...
	if (!result){
		MessageBox(hwnd, L"Could not initialize the normal shader object.", L"Error", MB_OK);
		return false;
	}
	...
	if (!m_Light) {
		MessageBox(hwnd, L"Could not initialize the normal object.", L"Error", MB_OK);
		return false;
	}

	// LightClass 객체 초기화
	m_Light->SetDiffuseColor(1.0f, 1.0f, 1.0f, 1.0f);
	m_Light->SetPosition(1.0f, -0.5f, 0.0f);
	m_Light->SetAmbientColor(0.05f, 0.05f, 0.05f, 0.05f);
...
bool GraphicsClass::Render(float rotation) {
	...
	result = m_NormalShader->Render(m_D3D->GetDeviceContext(), m_Model->GetIndexCount(), worldMatrix, viewMatrix, projectionMatrix, m_Model->GetTextureArray(),  m_Light->GetDiffuseColor(), m_Light->GetPosition(), m_Light->GetAmbientColor(), m_Camera->GetPosition());
	if (!result) { 
		return false;
	}
