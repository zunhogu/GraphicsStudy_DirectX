...
bool GraphicsClass::Initialize(int screenWidth, int screenHeight, HWND hwnd) {
	...
	if (!result){
		MessageBox(hwnd, L"Could not initialize the clipplane shader object.", L"Error", MB_OK);
		return false;
	}
...
bool GraphicsClass::Render() {
	...
	result = m_ClipShader->Render(m_D3D->GetDeviceContext(), m_Model->GetIndexCount(), worldMatrix, viewMatrix, projectionMatrix, m_Model->GetTexture(), clipPlane);
	if (!result) { 
		return false;
	}
