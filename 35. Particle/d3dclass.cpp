...
bool D3DClass::Initialize(int screenWidth, int screenHeight, bool vsync, HWND hwnd, bool fullscreen, float screenDepth, float screenNear)
{
	...
	// 스왑체인과 백버퍼를 연결하기 위해서는 Direct3D 장치의 렌더타겟 뷰를 백버퍼로 설정한다. 
	...
	// 이제 필요한 것은 깊이-스텐실 description을 작성하는 것이다.
	...
	// 파티클들이 서로 겹칠 때 서로 블렌딩하게 할 것이기 때문에 블렌드 상태를 파티클에 맞게 설정해야한다.
	// 파티클이 겹치면 서로의 색상을 더하는 additive 블렌딩을 사용한다. 
	// 그렇게 하려면 ScrBlend(들어오는 파티클 텍스처)를 D3D11_BLEND_ONE으로 하여 파티클 색상이 결과에 더해지도록 한다. 
	// 또한 DestBlend(텍스처가 쓰여지는 백버퍼) 역시 D3D11_BLEND_ONE으로 하여 이미 그려진 파티클에 씌워지도록 한다.
