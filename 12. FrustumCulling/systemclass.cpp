...
출처: https://ppparkje.tistory.com/30?category=489719 [빠재의 노트]
...
bool SystemClass::Frame()
{
	...
	// 매 프레임마다 PositionClass 객체의 프레임 시간을 갱신하다.
	m_Position->SetFrameTime(m_Timer->GetTime());

	// 프레임 시간을 갱신한 뒤 PositionClass의 현재 키보드 상태를 얻어와 움직임 관련 함수들을 호출한다.
	// 이 함수들은 카메라의 현재 위치를 갱신한다.
	...
	// 카메라의 새 회전값을 받아 GraphicsClass의 Frame 함수에 전달한다.
	m_Position->GetRotation(rotationY);

	// Graphics 객체의 프레임함수를 실행해서 새로운 입력을 갱신한다.
	result = m_Graphics->Frame(rotationY);
	if (!result){
		return false;
	}

	// Graphics 객체의 Render 함수를 통해 화면에 그려준다.
...
{
	// 이제 키보드 마우스 입력은 inputclass에서 처리하기때문에 윈도우에서 읽는 부분은 지워준다.
