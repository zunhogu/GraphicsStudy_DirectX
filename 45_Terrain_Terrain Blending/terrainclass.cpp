...
// LoadMaterialFile() 은 Material Map의 텍스처 파일을 읽고 모든 텍스처는 텍스처 배열에 저장되어 해당 배열에서 Material Group의 인덱스로 텍스처에 접근할 수 있다.
// 또 Material Group에 존재하는 정점 / 인덱스 버퍼를 생성하는 작업을 하기도 한다.
...
// LoadMaterialMap()은 MaterialMap.bmp 파일을 읽어서 rgb값을 가져온뒤 이를 heightmap의 rgb index에 저장한다.
// 해당 Material Group을 렌더링할때 이 index를 가지고 hegiht map의 특정 위치에 렌더링을 할수 있는 것이다. 
...
// LoadMaterialBuffers() 는 Material Group에 대해 정점 / 인덱스 버퍼를 만드는 함수다.
bool TerrainClass::LoadMaterialBuffers(ID3D11Device* device)
{
	...
	for (j = 0; j < (m_terrainHeight - 1); j++)
	{
		for (i = 0; i < (m_terrainWidth - 1); i++)
		{
			...
			// Upper left.
			m_Materials[index].vertices[vIndex].position = XMFLOAT3(m_heightMap[index3].x, m_heightMap[index3].y, m_heightMap[index3].z);
			m_Materials[index].vertices[vIndex].texture = XMFLOAT2(0.0f, 0.0f);
			m_Materials[index].vertices[vIndex].normal = XMFLOAT3(m_heightMap[index3].nx, m_heightMap[index3].ny, m_heightMap[index3].nz);
			m_Materials[index].vertices[vIndex].color = XMFLOAT4(m_heightMap[index3].r, m_heightMap[index3].g, m_heightMap[index3].b, 1.0f);
			m_Materials[index].indices[vIndex] = vIndex;
			vIndex++;

			// Upper right.
			m_Materials[index].vertices[vIndex].position = XMFLOAT3(m_heightMap[index4].x, m_heightMap[index4].y, m_heightMap[index4].z);
			m_Materials[index].vertices[vIndex].texture = XMFLOAT2(1.0f, 0.0f);
			m_Materials[index].vertices[vIndex].normal = XMFLOAT3(m_heightMap[index4].nx, m_heightMap[index4].ny, m_heightMap[index4].nz);
			m_Materials[index].vertices[vIndex].color = XMFLOAT4(m_heightMap[index4].r, m_heightMap[index4].g, m_heightMap[index4].b, 1.0f);
			m_Materials[index].indices[vIndex] = vIndex;
			vIndex++;

			// Bottom left.
			m_Materials[index].vertices[vIndex].position = XMFLOAT3(m_heightMap[index1].x, m_heightMap[index1].y, m_heightMap[index1].z);
			m_Materials[index].vertices[vIndex].texture = XMFLOAT2(0.0f, 1.0f);
			m_Materials[index].vertices[vIndex].normal = XMFLOAT3(m_heightMap[index1].nx, m_heightMap[index1].ny, m_heightMap[index1].nz);
			m_Materials[index].vertices[vIndex].color = XMFLOAT4(m_heightMap[index1].r, m_heightMap[index1].g, m_heightMap[index1].b, 1.0f);
			m_Materials[index].indices[vIndex] = vIndex;
			vIndex++;
			...
			// Bottom right.
			m_Materials[index].vertices[vIndex].position = XMFLOAT3(m_heightMap[index2].x, m_heightMap[index2].y, m_heightMap[index2].z);
			m_Materials[index].vertices[vIndex].texture = XMFLOAT2(1.0f, 1.0f);
			m_Materials[index].vertices[vIndex].normal = XMFLOAT3(m_heightMap[index2].nx, m_heightMap[index2].ny, m_heightMap[index2].nz);
			m_Materials[index].vertices[vIndex].color = XMFLOAT4(m_heightMap[index2].r, m_heightMap[index2].g, m_heightMap[index2].b, 1.0f);
			m_Materials[index].indices[vIndex] = vIndex;
			vIndex++;
...
// ReleaseMaterials()은 Material Group Array를 Relase 하는 함수다.
...
// Render()는 Material Group Array 요소들의 정점 및 인덱스 버퍼를 가지고 렌더링을 하는 함수다.
// 각 Mateirl Group을 살펴보고 해당 텍스처와 연결된 지형 정점 데이터를 그린다.
// 한 Group이 완료되면 Texture를 다음 그룹으로 변경한 다음 해당 그룹의 정점을 그린다.
// 모든 Group이 모두 렌더링 될때가지 지속된다.
bool TerrainClass::Render(ID3D11DeviceContext* deviceContext, Terrainshaderclass* shader, XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix, XMFLOAT4 ambientColor, XMFLOAT4 diffuseColor, XMFLOAT3 lightDirection)
{
	unsigned int stride;
	unsigned int offset;
	int i;
