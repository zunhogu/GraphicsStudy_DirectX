...
// SetFps 함수는 입력으로 정수를 받고 문자열로 변환한다.
// fps 값이 문자열로 변환되고 나면 이 숫자가 fps값이라는 것을 나타내는 다른 문자열 뒤에 붙인다
// 그 뒤에 화면에 그려지기 위해 문장 구조체에 저장된다.
// 이 함수 또한 fps가 60 이상이면 초록색 60미만이면 노란색 30 미만이면 붉은색 글자가 표시된다. 
...
// SetCpu 함수는 cpu값을 받고 문자열로 바꾼뒤 문장 구조체에 저장하고 그려진다.
