...
bool FoliageClass::Initialize(ID3D11Device* device, WCHAR* textureFilename, int fCount)
{
	...
	// GeneratePositions()를 통해서 임의의 폴리지 위치로 m_foliageArray를 로드한다. 또한 각 폴리지 조각에 대해 고유한 색상을 생성하고 Array에 모두 저장한다.
	...
	// InitializeBuffers() 함수로 위에서 채워넣은 m_folliageArray를 사용하여 폴리지를 렌더링하기 위한 인스턴스 버퍼를 생성한다.
...
// Frame 함수는 매 프레임마다 호출되어 바람의 방향과 바람의 회전에 대해 단풍의 회전을 업데이트한다.
// 카메라 위치도 고려되어 position, X rotation, Z rotation 으로 구성된 최종 행렬이 생성되고 m_instance에 저장된다. 모든 폴리지 회전이 계산되고 저장되면
// 이 프레임을 렌더링하는 데 사용되는 인스터스 버퍼에 m_instance 배열을 복사한다.
...
bool FoliageClass::InitializeBuffers(ID3D11Device* device)
{
	...
	// 단풍을 위한 단일 쿼드를 만든다. 위치와 텍스처 좌표만 있으면 된다. 인스턴싱을 사용하고 있기 때문에 각 단풍 조각에 대해 이 동일한 쿼드를 다시 렌더링할 것이다.
	...
	// 여기에서 인스턴스 버퍼를 설정한다. 위치와 색상도 인스턴스화하고 있다. 위치와 회전을 단일 행렬에 저장된다.
	// 이 행렬은 카메라 위치와 바람의 회전에 따라 매 프레임 업데이트된다. 처음에는 Frame 함수 동안 업데이트될 때까지 첫번째 인스턴스 버퍼를 렌더링하지 않을 것이 때문에 처음에는 단위행렬만 사용한다.
	// 그러나 색상은 동일하게 유지되므로, 색상 정보를 인스턴스 배열에 복사한다.
	// 인스턴스 배열은 초기 인스턴스 버퍼를 생성하는 데 사용된다. 매 프레임마다 인스턴스 배열에서 인스턴스 버퍼를 업데이트할 것이기 대문에 인스턴버스 버퍼의 사용을 동적으로 설정함에 유의하자
...
// GeneratePosition 함수는 석재 평면의 각 단풍 조각에 무작위로 위치를 할당한다.
// 또한 각 단풍 조각에 대해 임의의 색상을 지정한다. 이 데이터는 렌더링을 위해 각폴리지 조각을 인스턴스화 화는데 사용된다.
