...
// Render 함수에는 vertexCount와 instanceCount가 필요한데, 이는 인덱스 버퍼가 아닌 인스턴스 버퍼를 사용하여 렌더링하기 때문이다.
...
bool FoliageShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	result = D3DCompileFromFile(vsFilename, NULL, NULL, "FoliageVertexShader", "vs_5_0", 0, 0, &vertexShaderBuffer, &errorMessage);
	if(FAILED(result))
	{
	...
	result = D3DCompileFromFile(psFilename, NULL, NULL, "FoliagePixelShader", "ps_5_0", 0, 0, &pixelShaderBuffer, &errorMessage);
	if(FAILED(result))
	{
		// If the shader failed to compile it should have written something to the error message.
		if(errorMessage)
	...
	// 정점과 같이 폴리지 조각 고유한 World Matrix의 정보를 입력조립기로 줘야하기 때문에 Float4 를 4개를 생성한다.
