...
bool RefractionShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix, ID3D11ShaderResourceView* texture, XMFLOAT4 diffuseColor, XMFLOAT3 lightDirection, XMFLOAT4 ambientColor, XMFLOAT4 clipPlane) {

	bool result;
	// 렌더링에 사용할 셰이더의 인자를 입력한다.
	result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, texture, diffuseColor, lightDirection, ambientColor, clipPlane);
	if (!result) {
		return false;
	}
...
bool RefractionShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename) {
	...
	result = device->CreateBuffer(&clipPlaneBufferDesc, NULL, &m_clipingBuffer);
	if (FAILED(result)){
		return false;
	}
...
bool RefractionShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix, ID3D11ShaderResourceView* texture, XMFLOAT4 diffuseColor, XMFLOAT3 lightDirection, XMFLOAT4 ambientColor, XMFLOAT4 clipPlane) {
	HRESULT result;
	...
	// 클리핑 상수버퍼 초기화
	result = deviceContext->Map(m_clipingBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
	if (FAILED(result)) {
		return false;
	}
	...
	deviceContext->Unmap(m_clipingBuffer, 0);

	bufferNumber = 1;

	deviceContext->VSSetConstantBuffers(bufferNumber, 1, &m_clipingBuffer);
